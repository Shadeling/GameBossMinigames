...
namespace UTTools
{
    public abstract class PSYDictionaryDrawer<TK, TV> : PropertyDrawer
    {
        protected PSYDictionary<TK, TV> _Dictionary;
        ...
        protected virtual void CheckInitialize(SerializedProperty property, GUIContent label)
        {
            if (_Dictionary == null)
            {
                ...
                try
                {
                    _Dictionary = fieldInfo.GetValue(target) as PSYDictionary<TK, TV>;
                }
                catch (Exception)
                {
                    target = target.GetType().GetField(property.propertyPath.Substring(0, property.propertyPath.IndexOf('.'))).GetValue(target);
                }

                if (target.GetType().IsArray)
                {
                    var index = Convert.ToInt32(new string(property.propertyPath.Where(c => char.IsDigit(c)).ToArray()));
                    target = ((List<object>)target)[index];
                }
                ...
                if (_Dictionary == null)
                {
                    _Dictionary = new PSYDictionary<TK, TV>();
        ...
        private void AddNewItem()
        {
            ...
            else if (typeof(TK).BaseType == typeof(Enum))
            {
                int i = 0;
                do
                {
                ...
                while (_Dictionary.ContainsKey(key) && i < PSYTools.EnumSize(typeof(TK)));
            }
            else
            {
                key = default(TK);
